...
int main(int argc, char *argv[]) {
	...
	xwlan_permissions denied_permissions = xwlan_get_permission(ctx, required_permissions);
	if (xwlan_check_permission(ctx, denied_permissions, XWLAN_PERMISSION_NONE) != 1) {
	...
	for(size_t i = 0; i < interfaces_len; i++) {
		...
		if (wlan_if == NULL) {
			printf("ERROR: One of the enumerated wlan interface is null, disconnected?\n");
			continue;
		}
		printf("Interface[%zd]: \n", wlan_if->index);
